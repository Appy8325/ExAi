...
type Source = {
  ...
  source_type: string; file_id: string | null; storage_key: string | null;
...
async function loadSource(sourceId: string) {
  const rows = (await db.execute(sql<Source>`
    SELECT source.id, source.event_id, source.organizer_organization_id,
      ...
      source.source_type, source.file_id, file.storage_key, file.content_type,
      file.byte_size
...
async function downloadObject(path: string) {
  const url = process.env.WORKER_SUPABASE_URL ?? process.env.API_SUPABASE_URL;
  const key = process.env.WORKER_SUPABASE_SERVICE_ROLE_KEY ?? process.env.API_SUPABASE_SERVICE_ROLE_KEY;
  ...
  if (bytes.length > MAX_KNOWLEDGE_UPLOAD_BYTES) throw new Error("Stored file exceeds the 25 MB ingestion limit.");
  return bytes;
}
