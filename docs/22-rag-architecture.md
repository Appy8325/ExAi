...
> **Phase 4 provider amendment:** the implemented embedding provider is NVIDIA NIM only, using `nvidia/nv-embedqa-e5-v5` through `/v1/embeddings` with 1024-dimensional passage/query vectors. Earlier Voyage references below are superseded; no Voyage runtime dependency or credential remains.
...
**Ranking:** `RetrievalService.search` embeds the query with NVIDIA NIM, applies tenant and visibility filters before vector comparison, orders candidates by pgvector cosine distance, and returns the requested top results (default 8, max 12). If the candidate set is empty, Copilot's grounding rule (doc 21 §3.1) requires the model to say it can't find grounded information rather than answer un-cited.
