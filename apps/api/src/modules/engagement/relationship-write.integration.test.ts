...
describe("relationship write repositories", () => {
  ...
  it("enrolls authenticated attendees through the real relationship engine without cross-booth access", async () => {
    ...
    const service = new PlatformEnrollmentService(client as never, auth as never, new LeadSubmissionsService(new LeadSubmissionsRepository(client as never), { enrich: () => Promise.resolve() } as never), {} as never, {} as never, {} as never, {} as never);
