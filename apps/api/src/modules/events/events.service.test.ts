...
describe("EventsService", () => {
  ...
  it("rejects invalid event values", () => {
    ...
    expect(() =>
      service.create({ ...validInput, timezone: "not/a-timezone" }),
    ).toThrow(BadRequestException);
    expect(() =>
      service.create({ ...validInput, endAt: validInput.startAt }),
    ).toThrow(BadRequestException);
  ...
  it("updates and archives through the organization-scoped repository methods", async () => {
    const repository = {
      ...
      archive: vi
        .fn()
        .mockResolvedValue({ id: "event-id", status: "archived" }),
    };
    const service = new EventsService(repository as never);
    ...
    await expect(
      service.archive("organization-id", "event-id", "user-id"),
    ).resolves.toEqual({ id: "event-id", status: "archived" });
  ...
  it("publishes only a configured draft event", async () => {
    const repository = {
      ...
      publish: vi
        .fn()
        .mockResolvedValue({ id: "event-id", status: "published" }),
    };
    const service = new EventsService(repository as never);

    await expect(
      service.publish("organization-id", "event-id", "user-id"),
  ...
  it("blocks publication without the required privacy policy", async () => {
    const service = new EventsService({
      findById: vi
        .fn()
        .mockResolvedValue({
