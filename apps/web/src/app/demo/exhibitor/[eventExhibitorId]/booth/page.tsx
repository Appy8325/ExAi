  ...
  return (
    <div className="mx-auto max-w-5xl px-6 py-12 sm:px-10">
      ...
      <header className="mt-4">
        ...
        {publicBooth && (
          <p className="mt-1 text-body text-secondary">{publicBooth.boothName}</p>
        )}
      </header>

      {publicBooth?.description ? (
        <section className="mt-6">
          <h2 className="text-title-sm font-semibold text-primary">Description</h2>
          <p className="mt-1 text-body text-secondary">{publicBooth.description}</p>
        </section>
      ) : null}
      ...
      {publicBooth?.leadForm ? (
        <section className="mt-8">
          ...
          <p className="mt-1 text-caption text-muted">{publicBooth.leadForm.name} &mdash; read-only preview</p>
          {publicBooth.leadForm.consentText ? (
            <p className="mt-2 text-body italic text-secondary">{publicBooth.leadForm.consentText}</p>
          ) : null}
          <ul className="mt-4 space-y-2">
            {publicBooth.leadForm.fields.map((field) => (
              <li key={field.key} className="rounded-xl border border-default bg-surface px-4 py-3">
                ...
                <span className="ml-2 rounded bg-sunken px-1.5 py-0.5 text-caption text-muted">{field.type}</span>
                {field.helpText ? <p className="mt-0.5 text-caption text-muted">{field.helpText}</p> : null}
              </li>
            ))}
          </ul>
        </section>
      ...
      {publicBooth?.resources && publicBooth.resources.length > 0 ? (
        <section className="mt-8">
          <h2 className="text-title-sm font-semibold text-primary">Published resources</h2>
      ...
      {booth.publicQrToken ? (
        <div className="mt-8">
          <Link
            ...
            className="inline-flex items-center gap-1 rounded-lg bg-status-success-solid px-4 py-2 text-body font-semibold text-on-brand"
          >
